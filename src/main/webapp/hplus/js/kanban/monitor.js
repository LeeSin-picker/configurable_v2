...
$(function(){
		//---------------------------------------------------lmyAdd
	//火狐50或51版本，低于52版本，因为52版本不支持NPAPI，谷歌45版本以下，45之上不支持NPAPI  必须是32位浏览器；		
	//谷歌启用NPAPI方法：chrome://flags/#enable-npap（地址），chrome://plugins/，谷歌要打开Web Components
	
	clickLogin();
	changeWndNum('1x1');
	
	clickStartRealPlay();
setTimeout(clickStartRealPlay,2000)
...
// 登录
function clickLogin() {
	var szIP = '192.168.200.4',//$("#loginip").val(),
		szPort =80,//$("#port").val(),
		szUsername ='admin',//$("#username").val(),
		szPassword ='12345';// $("#password").val();
...
function clickStartRealPlay() {
	...
	var oWndInfo = WebVideoCtrl.I_GetWindowStatus(g_iWndIndex),
		szIP = '192.168.200.4',// $("#ip").val(),
		iStreamType = parseInt(1, 10),//parseInt($("#streamtype").val(), 10),
		iChannelID = parseInt(2, 10),//可控制显示的监控位置
